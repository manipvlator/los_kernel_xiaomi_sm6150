...
rename_hook_done:
	...
	ret = arm64_bl_patch(kallsyms_lookup_retry("bprm_execve"), 256 * sizeof(void *), symbol_addr, (uintptr_t)&ksu_bprm_check);
	if (ret)
		ret = arm64_bl_patch(kallsyms_lookup_retry("search_binary_handler"), 256 * sizeof(void *), symbol_addr, (uintptr_t)&ksu_bprm_check);
	if (ret)
